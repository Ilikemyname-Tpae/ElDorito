...
dew.on("show", function(e){
    if(settingsArray['Game.SkipTitleSplash'] == 1){
        ...
        $("#blackLayer").hide();
        $.getJSON( "https://raw.githubusercontent.com/UnofficialElDewrito/UnofficialElDewrito/master/currentRelease.json", function(data) {
            dew.getVersion().then(function (version) {
                if(parseVersion(data.release[0].version) != parseVersion(version)) {
                    dew.show('alert',{"title":"Update Available!", "body":"There is a newer version of Unofficial ElDewrito available.|r|n|r|nWould you like to launch the updater?", "type":"update"});
                }
            });
        }); 
        dew.command('Game.FirstRun', {}).then(function(result){
            if(result == 1){
                $.getJSON("https://raw.githubusercontent.com/UnofficialElDewrito/UnofficialElDewrito/master/announcements.json", function(data) {
